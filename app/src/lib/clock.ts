...
/**
 * 
 * @param assumed Digits assumed to be in the number (for example, 5 would be the first 5 digits)
 */
export const nowSmall = (assumed: number) => {
    const now = Date.now().toString();
    return {
        time: +now.slice(assumed),
        rebuild: (time: number) => {
            const str = time.toString();
            return new Date(
                +(now.slice(now.length - str.length) + str)
            );
        }
    }
};